...
import { EyeIcon, SquareKanbanIcon, TrashIcon } from "lucide-react";
...
// NOTE: Currently hard-coded, can be fetched from API in future
...
const EditAssessmentTests = ({ insideModal = false }) => {
  ...
  // Default library to platform on mount / when bootstrap loads
  useEffect(() => {
  ...
  const selectedSkillTags =
    searchParams?.tags__in?.filter((t) => skillTags.some((st) => st.id === t)) || [];
  const selectedTestTypeTags =
    searchParams?.tags__in?.filter((t) => testTypeTags.some((tt) => tt.id === t)) || [];

  const tests = assessmentTestsData?.pages?.flatMap((page) => page.results) || [];
  const count = assessmentTestsData?.pages?.[0]?.count || 0;

  // Build selected tests map for quick look-ups
      ...
      <Section variant="white" className="p-3" contentClassName="flex flex-col gap-5">
        <div className="flex flex-row gap-3 justify-between">
          {Array.from({ length: MAX_TEST_COUNT }).map((_, index) => {
            const test = selectedTests.length > index ? selectedTests[index] : null;
            return (
              <>
            ...
            <Dropdown
              ...
              onChange={(val) => {
                setSearchParams((prev) => {
                  ...
                  const newTags = [
                    ...(tags__in || []).filter((t) => !skillTags.some((st) => st.id === t)),
                    ...val,
                  ];
                ...
                <>
                  ...
                  <AssessmentTestCard
                    ...
                    tags={TEST_TAGS.map((t) => ({
                      text: t,
                      icon: ICON_MAP[t],
                      variant: t === "Recommended" ? "outlinePrimary" : "outline",
                    }))}
...
export default React.memo(EditAssessmentTests);
