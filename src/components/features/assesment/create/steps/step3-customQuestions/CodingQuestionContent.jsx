...
//             const currentStubs = watch('codeStubs') || {};
//             const newStubs = { ...currentStubs };
//             console.log(newStubs)
//             let changed = false;
//             const selectedLangObjs = selectedLanguages.map(item =>
//                 typeof item === 'object' ? item : allLanguages.results.find(lang => lang.id === item)
//             ).filter(Boolean)

//             for (const lang of selectedLangObjs) {
//                 if (newStubs[lang.name] === undefined) {
//                     newStubs[lang.name] = lang.defaultTemplate || lang.default_template?.content || ''
//                 }
//             }
...
//             // });
//             Object.keys(newStubs).forEach(langName => {
//                 if (!selectedLangObjs.some(lang => lang.name === langName)) {
//                     delete newStubs[langName];
//                     changed = true
//                 }
//             })
...
//     }) {
//         useEffect(() => {
//             console.log('codestubs from step3', codeStubs)
//         })
...
//             );
//         // const codeStubs = watch('codeStubs') || {};
...
//                 isLanguagesLoading={isLanguagesLoading}
//                 languagesError={languagesError}
//                 selectedLanguages={getValues('selectedLanguages')}
//                 langOverrides={langOverrides}
//                 codeStubs={watch('codeStubs' || {})}
//             />;
...
const CodingQuestionContent = ({
    ...
    useEffect(() => {
        if (allLanguages && selectedLanguages && selectedLanguages.length > 0) {
            const currentStubs = form.getValues('codeStubs') || {};
            const newStubs = { ...currentStubs };
            console.log(newStubs)
            let changed = false;
            const selectedLangObjs = selectedLanguages.map(item =>
            ...
            if (changed) {
                form.setValue('codeStubs', newStubs, { shouldDirty: true, shouldTouch: true, shouldValidate: false });
            }
        }
    }, [allLanguages, selectedLanguages, form]);
    ...
    function Step1Content({ form }) {
        const { register, control, formState: { errors }, clearErrors } = form;
        return (
            <div className="flex flex-col gap-6">
                <div className="flex items-center gap-2">
                    <h3 className="flex text-base font-medium text-greyPrimary">Problem Statement<span className="text-dangerPrimary">*</span></h3>
    ...
    // Step2Content is now a pure component
    function Step2Content({ form, allLanguages, isLanguagesLoading, languagesError, langOverrides, setLangOverrides }) {
        ...
        let filteredLangs = langResultMap?.filter(lang => lang.name.toLowerCase().includes(searchValue.toLowerCase()));
        if (showPopularOnly) {
            filteredLangs = filteredLangs?.filter(lang => lang.isPopular);
        }
        return (
    ...
    // Step3Content is now a pure component
    function Step3Content({ form, allLanguages, isLanguagesLoading, languagesError, selectedLanguages, langOverrides }) {
        // const codeStubs = form.watch('codeStubs') || {};
        ...
        // Force re-render when codeStubs changes
        useEffect(() => {
            console.log('codestubs from step3', codeStubs);
        }, [codeStubs]);
    ...
    function renderStepContent() {
        switch (currentStep) {
            case 1: return <Step1Content form={form} />;
            case 2: return <Step2Content form={form} allLanguages={allLanguages} isLanguagesLoading={isLanguagesLoading} languagesError={languagesError} langOverrides={langOverrides} setLangOverrides={setLangOverrides} />;
            case 3: return <Step3Content form={form} allLanguages={allLanguages} isLanguagesLoading={isLanguagesLoading} languagesError={languagesError} selectedLanguages={form.getValues('selectedLanguages')} langOverrides={langOverrides} />;
            case 4: return <Step4Content form={form} testCases={testCases} setTestCases={setTestCases} />;
            default: return null;
        }
    }
    return (
